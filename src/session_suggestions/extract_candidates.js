...
exports.DEFAULT_MAX_CANDIDATES = void 0;
exports.parseSessionTranscript = parseSessionTranscript;
exports.suggestSessionHabitCandidates = suggestSessionHabitCandidates;
exports.parseCandidateReference = parseCandidateReference;
exports.validateSuggestionSnapshot = validateSuggestionSnapshot;
exports.findSuggestedCandidate = findSuggestedCandidate;
const context_rules_1 = require("../habit_core/context_rules");
...
exports.DEFAULT_MAX_CANDIDATES = 5;
function clampScore(value) {
    return Math.max(0, Math.min(1, Number(Number(value).toFixed(2))));
}
function buildConfidenceDetails({ sourceType, baseScore, adjustments = [], finalScore, summary }) {
    return {
...
function normalizePhrase(value) {
    return (0, context_rules_1.normalizeText)(String(value || ""));
}
function canonicalizeRole(label) {
    const normalized = (0, context_rules_1.normalizeText)(label);
...
function buildKnownPhraseSet(registryPath) {
    const mergedRules = (0, user_registry_1.loadMergedHabits)((0, interpreter_1.loadDefaultHabits)(), registryPath);
    const state = (0, user_registry_1.loadUserRegistryState)(registryPath);
...
function extractExplicitDefinitionRule(message) {
    ...
    for (const entry of patterns) {
        ...
        (0, validate_registry_1.validateHabitRules)([rule]);
        return {
            rule,
...
function extractExplicitCandidates(messages, knownPhrases) {
    ...
    for (const message of messages) {
        ...
        try {
            parsedRequest = (0, management_prompt_1.parseHabitManagementRequest)(message.content);
        }
        catch {
            parsedRequest = null;
        }
        ...
        addOrReplaceCandidate(candidates, explicitRule.phrase, {
            ...
            confidence_details: buildConfidenceDetails({
                ...
                summary: explicitDefinition.correction_count > 0
...
function extractRepeatedPhraseCandidates(messages, knownPhrases, explicitCandidates) {
    ...
    for (const message of messages) {
        ...
        let parsedRequest = null;
        try {
            parsedRequest = (0, management_prompt_1.parseHabitManagementRequest)(content);
        }
        catch {
            parsedRequest = null;
        }
...
function suggestSessionHabitCandidates(transcriptText, options = {}) {
    const registryPath = options.userRegistryPath || user_registry_1.USER_REGISTRY_PATH;
    const maxCandidates = Number.isInteger(options.maxCandidates)
        ? options.maxCandidates
        : exports.DEFAULT_MAX_CANDIDATES;
