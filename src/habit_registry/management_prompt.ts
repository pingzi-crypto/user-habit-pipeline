...
type PathRequestAction = "import" | "export";

interface PathRequestConfig {
...
export type HabitManagementRequest =
  | AddHabitManagementRequest
  | ApplyCandidateHabitManagementRequest
  | ExportHabitManagementRequest
  | IgnoreCandidateHabitManagementRequest
  | IgnorePhraseHabitManagementRequest
  | ImportHabitManagementRequest
  | ListHabitManagementRequest
  | RemoveHabitManagementRequest
  | SuggestHabitManagementRequest;

function parseListRequest(message: string): ListHabitManagementRequest | null {
...
function parseSuggestRequest(message: string): SuggestHabitManagementRequest | null {
  if (
...
function parseApplyCandidateOverrides(segmentText: string): ApplyCandidateOverrides {
  const overrides: ApplyCandidateOverrides = {};
...
function parseApplyCandidateRequest(message: string): ApplyCandidateHabitManagementRequest | null {
  const patterns = [
...
function parseIgnoreCandidateRequest(message: string): IgnoreCandidateHabitManagementRequest | null {
  const patterns = [
...
function parseIgnorePhraseRequest(message: string): IgnorePhraseHabitManagementRequest | null {
  const patterns = [
...
function parseRemoveRequest(message: string): RemoveHabitManagementRequest | null {
...
function parseKeyValueSegments(segmentText: string): Record<string, string> {
...
): ImportHabitManagementRequest | ExportHabitManagementRequest | null {
...
function parseAddRequest(message: string): AddHabitManagementRequest | null {
  ...
  if (matchType === "exact" || matchType === "substring") {
...
function parseImportRequest(message: string): ImportHabitManagementRequest | null {
  return parsePathRequest(message, {
    action: "import",
    label: "Import-habit",
    pattern: /^\s*(?:导入|导进去|载入)(?:用户)?习惯短句(?:[:：]|\s)\s*(.+?)\s*$/u
  }) as ImportHabitManagementRequest | null;
}

function parseExportRequest(message: string): ExportHabitManagementRequest | null {
  return parsePathRequest(message, {
    action: "export",
    label: "Export-habit",
    pattern: /^\s*(?:导出|导出来|备份)(?:用户)?习惯短句(?:[:：]|\s)\s*(.+?)\s*$/u
  }) as ExportHabitManagementRequest | null;
}

export function parseHabitManagementRequest(message: string): HabitManagementRequest | null {
