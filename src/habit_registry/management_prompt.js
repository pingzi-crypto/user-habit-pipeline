...
exports.parseHabitManagementRequest = parseHabitManagementRequest;
...
function parseSuggestRequest(message) {
    if (/^\s*(?:扫描|整理|看看|查看)(?:这次|当前)?(?:会话|对话|线程)(?:里|中)?(?:的)?(?:习惯候选|常用短句|纠正说法|用户习惯候选)\s*$/u.test(message)
...
function parseAddRequest(message) {
    ...
    if (!phrase || !normalizedIntent) {
        throw new Error("Add-habit prompt requires at least phrase/短句 and intent/意图 fields.");
    }
    const scenarioBias = toStringArray(fields.scenario || fields["场景"]);
    const preferredTerms = toStringArray(fields.preferred_terms || fields.preferred || fields["术语"]);
    const notes = toStringArray(fields.notes || fields["备注"]);
    const disambiguationHints = toStringArray(fields.disambiguation_hints || fields["澄清提示"]);
