...
import {
  ...
  type ApplyCandidateHabitManagementRequest,
  type ApplyCandidateOverrides,
  type HabitManagementRequest
...
interface SuggestionCandidate {
  candidate_id: string;
  phrase: string;
  suggested_rule?: (Partial<HabitRule> & { phrase: string; normalized_intent?: string }) | null;
...
interface SuggestSessionResult extends SuggestionSnapshot {
...
const {
  ...
  findSuggestedCandidate: (snapshot: SuggestionSnapshot, reference: string) => SuggestionCandidate;
  suggestSessionHabitCandidates: (
...
function createDefaultArgs(): ManageCliArgs {
  return {
    action: null,
    phrase: null,
    intent: null,
...
export function parseArgs(argv: string[]): ManageCliArgs {
...
function readRequestFromStdin(): string {
...
function parseJsonPayload<T = unknown>(text: string, sourceLabel: string): T {
...
function readTranscriptInput(args: ManageCliArgs): string {
...
function readSuggestionsInput(args: ManageCliArgs): SuggestionSnapshot {
  if (args.suggestionsFromStdin) {
    ...
    return parseJsonPayload<SuggestionSnapshot>(trimmed, "--suggestions-stdin input");
  }
  ...
  if (args.suggestionsPath) {
    ...
    return parseJsonPayload<SuggestionSnapshot>(trimmed, "--suggestions file");
  }

  const cached = loadSuggestionSnapshot<SuggestionSnapshot>(args.registryPath);
  return cached.snapshot;
}

function resolveScenarioBias(
  baseRule: Partial<HabitRule>,
  args: ManageCliArgs,
  overrides: ApplyCandidateOverrides = {}
): string[] {
...
function buildRuleFromSuggestedCandidate(
  candidate: SuggestionCandidate,
  args: ManageCliArgs,
  overrides: ApplyCandidateOverrides = {}
): HabitRule {
  const baseRule: Partial<HabitRule> & { phrase: string; normalized_intent?: string } = candidate.suggested_rule
...
function resolveImportMode(mode: string | null | undefined): ImportMode {
  return mode === "merge" ? "merge" : "replace";
}

export function executeStructuredAction(args: ManageCliArgs): ManageCliOutput {
  if (args.action === "list") {
...
function isApplyCandidateRequest(request: HabitManagementRequest): request is ApplyCandidateHabitManagementRequest {
  return request.action === "apply-candidate";
}

export function executeRequestAction(args: ManageCliArgs): ManageCliOutput {
