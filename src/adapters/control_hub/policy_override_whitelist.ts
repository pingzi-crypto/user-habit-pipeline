...
export const CONTROL_HUB_POLICY_OVERRIDE_WHITELIST: PolicyOverrideWhitelistEntry[] = [
  {
    phrase: "这次例外",
    variants: ["这次例外", "本次例外"],
...
export function findWhitelistMatches(rawUserText: string): PolicyOverrideWhitelistEntry[] {
  const normalized = normalizePolicyOverrideText(rawUserText);
  return CONTROL_HUB_POLICY_OVERRIDE_WHITELIST.filter((entry) =>
    entry.variants.some((variant) => normalized.includes(normalizePolicyOverrideText(variant)))
  );
}

export function normalizePolicyOverrideText(value: string): string {
