...
import { normalizeText } from "./context_rules";
import { scoreCandidate, shouldAskClarifyingQuestion } from "./scoring";
import type {
  CandidateRule,
  HabitInput,
  HabitOutput,
...
const { validateHabitRules } = require("../habit_registry/validate_registry") as {
  validateHabitRules: (rules: HabitRule[]) => HabitRule[];
};
const { loadMergedHabits, USER_REGISTRY_PATH } = require("../habit_registry/user_registry") as {
  loadMergedHabits: (defaultRules: HabitRule[], registryPath?: string) => HabitRule[];
  USER_REGISTRY_PATH: string;
};

export const DEFAULT_REGISTRY_PATH = path.join(__dirname, "..", "habit_registry", "default_habits.json");

let cachedRules: HabitRule[] | null = null;

export function loadHabitsFromFile(registryPath: string): HabitRule[] {
  const raw = fs.readFileSync(registryPath, "utf8");
  return validateHabitRules(JSON.parse(raw));
}

export function loadDefaultHabits(): HabitRule[] {
  if (!cachedRules) {
...
export function createUnknownOutput(): HabitOutput {
  return {
...
function compareCandidates(left: CandidateRule, right: CandidateRule): number {
  const matchTypeWeight: Record<MatchType, number> = { exact: 2, substring: 1 };
...
export function findCandidateRules(message: string, rules: HabitRule[]): CandidateRule[] {
  ...
  const exactMatches = rules
    ...
    .map((rule): CandidateRule => ({ rule, matchType: "exact" }));
  ...
  return rules
    ...
    .map((rule): CandidateRule => ({ rule, matchType: "substring" }))
    .sort(compareCandidates);
}

function buildDisambiguationHints(topCandidate: ScoredCandidate | undefined, shouldClarify: boolean): string[] {
...
function buildNotes(topCandidate?: ScoredCandidate): string[] {
  if (!topCandidate) {
...
export function interpretHabit(input: HabitInput, options: InterpretHabitOptions = {}): HabitOutput {
